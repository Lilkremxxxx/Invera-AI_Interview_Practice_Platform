...
const Signup = () => {
  ...
  const copy = {
    ...
    signupRequired:
      language === 'vi'
        ? 'Tài khoản này chưa có trong hệ thống. Hãy đăng ký trước rồi mới đăng nhập.'
        : 'This account does not exist in the system yet. Sign up first, then log in.',
    oauthSignupRequired:
      language === 'vi'
        ? 'Tài khoản này chưa tồn tại trong hệ thống. Hãy đăng ký trước khi dùng Google hoặc GitHub để đăng nhập.'
        : 'This account does not exist in the system yet. Sign up before using Google or GitHub to log in.',
  ...
  const noticeMessage = notice === 'oauth-signup-required'
    ? copy.oauthSignupRequired
    : notice === 'login-signup-required'
      ? copy.signupRequired
      : '';
          ...
          <div className="bg-card border border-border/60 rounded-2xl p-8 sm:p-10 shadow-lg">
            ...
            <form onSubmit={handleSubmit} className="space-y-6">
              {noticeMessage && !errorMsg && (
                <div className="flex items-start gap-2.5 rounded-lg border border-info/30 bg-info/10 p-3.5 text-sm text-info">
            ...
            <div className="mt-8 pt-8 border-t border-border/60">
              ...
              <div className="grid grid-cols-2 gap-3">
                <Button 
                  ...
                  onClick={() => authApi.oauthRedirect('google', 'signup')}
                >
                ...
                <Button 
                  ...
                  size="lg" 
                  className="h-12 border-border/80 hover:bg-accent/5 hover:border-accent/50 transition-all"
                  onClick={() => authApi.oauthRedirect('github', 'signup')}
                >
