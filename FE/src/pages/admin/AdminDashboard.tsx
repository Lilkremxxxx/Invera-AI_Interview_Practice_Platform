...
export function AdminDashboard() {
  ...
  const statCards = [
    {
      ...
      icon: Users,
      desc: `${safeStats.total_admins} ${copy.admins}`,
      path: '/admin/users'
    },
    {
      title: copy.activeUsers,
      value: safeStats.active_users ?? 0,
      icon: Users,
      desc: copy.activeUsersDesc,
      path: '/admin/users'
    },
    {
      title: 'Sessions',
      ...
      icon: FileText,
      desc: `${safeStats.completed_sessions} ${copy.completed}`,
      path: '/admin/sessions'
    },
    {
      title: copy.totalRevenue,
      value: formatRevenue(safeStats.total_revenue ?? 0),
      icon: DollarSign,
      desc: copy.revenueDesc,
      path: '/admin/revenue'
    },
    {
      title: 'Questions',
      ...
      icon: Target,
      desc: copy.questionsAvailable,
      path: null
    },
    {
      title: copy.answers,
      value: safeStats.total_answers,
      icon: CheckCircle,
      desc: `${copy.avgScore}: ${formatScore(safeStats.avg_score ?? 0)}`,
      path: null
    },
  ];
  ...
  const sessionStatusData = [
    { name: language === 'vi' ? 'Đã hoàn thành' : 'Completed', value: safeStats.completed_sessions, color: '#10b981' },
    { name: language === 'vi' ? 'Đang hoạt động' : 'Open', value: Math.max(safeStats.total_sessions - safeStats.completed_sessions, 0), color: '#6366f1' },
  ];
  ...
  return (
    <div className="space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-500">
      ...
      <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-6">
        {statCards.map((card, i) => (
          <Card
            ...
            className={`border-border/50 bg-card/50 backdrop-blur-sm shadow-sm transition-all duration-300 hover:-translate-y-1 hover:shadow-lg ${
              card.path
          ...
          >
            ...
            <CardContent>
              <div className="text-2xl font-bold text-accent tracking-tight">{card.value}</div>
      ...
      <div className="grid gap-6 xl:grid-cols-[1.15fr_0.85fr]">
        <Card className="border-border/50 shadow-sm hover:shadow-md hover:border-accent/30 transition-all duration-300">
          <CardHeader>
            <CardTitle>{copy.recentGrowth}</CardTitle>
            <CardDescription>{copy.recentGrowthDescription}</CardDescription>
          </CardHeader>
          <CardContent>
            <div className="h-72 bg-background/30 rounded-xl border border-border/30 p-4">
              <ResponsiveContainer width="100%" height="100%">
                <AreaChart data={recentUsersChart} margin={{ top: 10, right: 10, left: 0, bottom: 0 }}>
                  <defs>
                  ...
                  />
                  <Tooltip
                    cursor={{ stroke: 'rgba(20, 184, 166, 0.2)', strokeWidth: 1.5 }}
                      ...
                      if (active && payload && payload.length) {
                        return (
                          <div className="bg-popover/90 backdrop-blur-md border border-border/50 p-3 rounded-lg shadow-xl text-xs space-y-1">
                            ...
                            <p className="font-black text-[#14b8a6] text-sm">
                              {payload[0].value} {language === 'vi' ? 'đăng ký' : 'signups'}
                            </p>
                          </div>
                        );
                      }
                  ...
                  <Area
                    ...
                    activeDot={{ r: 6, fill: '#14b8a6', stroke: 'white', strokeWidth: 1.5 }}
                  />
                </AreaChart>
              </ResponsiveContainer>
            </div>
            <div className="mt-6 space-y-4">
              {users.slice(0, 6).map((u) => (
                <div key={u.id} className="flex items-center justify-between border-b border-border pb-4 last:border-0 last:pb-0 hover:bg-muted/10 p-2 rounded-xl transition-all duration-200">
                  <div>
        ...
        <div className="grid gap-6">
          <Card className="border-border/50 shadow-sm hover:shadow-md hover:border-accent/30 transition-all duration-300">
            ...
            <CardContent>
                    ...
                    <Tooltip
                      content={({ active, payload }) => {
                        if (active && payload && payload.length) {
                          ...
                          return (
                            <div className="bg-popover/90 backdrop-blur-md border border-border/50 p-3 rounded-lg shadow-xl text-xs space-y-1">
                              <p className="font-semibold text-muted-foreground">{payload[0].name}</p>
                              <p className="font-black text-sm" style={{ color: data.color }}>
                                {payload[0].value} {language === 'vi' ? 'phiên' : 'sessions'}
                              </p>
                            </div>
                          );
                        }
              ...
              <div className="grid grid-cols-2 gap-3 text-sm">
                {sessionStatusData.map((item) => (
                  <div key={item.name} className="rounded-2xl border border-border/70 bg-muted/30 px-4 py-3 hover:border-accent/30 transition-all duration-300">
          ...
          <Card className="border-border/50 shadow-sm hover:shadow-md hover:border-accent/30 transition-all duration-300">
            ...
            <CardContent className="space-y-8">
              <div className="h-64 bg-background/30 rounded-xl border border-border/30 p-4">
                <ResponsiveContainer width="100%" height="100%">
                  <BarChart data={questionRoleData} barCategoryGap={18} margin={{ top: 10, right: 10, left: 0, bottom: 0 }}>
                    <defs>
                      <linearGradient id="questionBarGrad" x1="0" y1="0" x2="0" y2="1">
                    ...
                    <Tooltip
                      ...
                      content={({ active, payload, label }) => {
                        if (active && payload && payload.length) {
                          return (
                            <div className="bg-popover/90 backdrop-blur-md border border-border/50 p-3 rounded-lg shadow-xl text-xs space-y-1">
                              <p className="font-semibold text-muted-foreground capitalize">{label}</p>
                              <p className="font-black text-[#14b8a6] text-sm">
                                {payload[0].value} {language === 'vi' ? 'câu hỏi' : 'questions'}
                              </p>
                            </div>
                          );
                        }
                    ...
                    />
                    <Bar dataKey="count" fill="url(#questionBarGrad)" radius={[6, 6, 0, 0]} />
                  </BarChart>
                </ResponsiveContainer>
              </div>
              ...
              <div className="grid gap-3 sm:grid-cols-2">
                {questionLevelData.filter((item) => item.count > 0).map((item) => (
                  <div key={item.level} className="rounded-2xl border border-border/70 bg-card px-4 py-3 hover:border-accent/30 transition-all duration-300">
